 ...
 * 
 * NOTE: Disabled due to event loop blocking issues on Fly.io
 * Re-enable when running on higher-memory/higher-CPU instance
 */
async function downloadWithParallelRanges(downloadUrl, timeout = 2 * 60 * 60 * 1000) {
  // DISABLED: Parallel downloads were blocking HTTP server health checks
  // For now, always fallback to single-connection streaming for stability
  return null;
}
...
async function stageFileToS3(downloadUrl, filename) {
  ...
  console.log(`📥 Downloading from Filemail: ${filename}`);
